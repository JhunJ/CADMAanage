...
def _drawing_color_to_aci(draw_color):
    """System.Drawing.Color 또는 Rhino 색 -> 가장 가까운 ACI 0-255. ByLayer(256)은 0 반환."""
...
def _add_entity_from_wkt_to_doc(doc, entity_type, wkt, layer_name, layer_index, props, aci_color=None, color_from_layer=True, out_ids=None):
    ...
    if m_lin:
        try:
            ...
            if entity_type == "HATCH" and closed:
                ...
                try:
                    pattern_index = doc.HatchPatterns.Find("Solid", True)
                    if pattern_index < 0:
                        pattern_index = doc.HatchPatterns.Find("Grid", True)
                    if pattern_index < 0 and doc.HatchPatterns.Count > 0:
                        pattern_index = 0
                    if pattern_index >= 0:
                        hatches = Rhino.Geometry.Hatch.Create(pc, pattern_index, 0, 1.0)
        ...
        for part in re.split(r"\)\s*,\s*\(", inner):
            ...
            try:
                ...
                if len(pts) >= 2:
                    ...
                    pl = Rhino.Geometry.Polyline(pts3)
                    pc = Rhino.Geometry.PolylineCurve(pl)
                    if entity_type == "HATCH" and len(pts3) >= 3:
                        closed_curves.append(pc)
                    else:
                        open_parts.append(pc)
            except (ValueError, TypeError):
                pass
        if entity_type == "HATCH" and closed_curves:
            try:
                pattern_index = doc.HatchPatterns.Find("Solid", True)
                if pattern_index < 0:
                    pattern_index = doc.HatchPatterns.Find("Grid", True)
                if pattern_index < 0 and doc.HatchPatterns.Count > 0:
                    pattern_index = 0
                if pattern_index >= 0:
                    hatches = Rhino.Geometry.Hatch.Create(closed_curves, pattern_index, 0, 1.0)
                    if hatches is not None:
                        for hatch in hatches:
                            g = doc.Objects.AddHatch(hatch)
                            if g is not None and str(g) != "00000000-0000-0000-0000-000000000000":
    ...
    # POLYGON (외곽선 닫힌 링, 다중 링 지원: POLYGON((외곽),(내곽),...)) — HATCH면 해치 객체로, 아니면 PolylineCurve
                    ...
                    if entity_type == "HATCH":
                        try:
                            ...
                            if pattern_index < 0 and doc.HatchPatterns.Count > 0:
                                pattern_index = 0
                            if pattern_index >= 0:
                                hatches = Rhino.Geometry.Hatch.Create(curves, pattern_index, 0, 1.0)
                                if hatches is None and len(curves) == 1:
                                    hatches = Rhino.Geometry.Hatch.Create(curves[0], pattern_index, 0, 1.0)
                                if hatches is not None:
                                    for hatch in hatches:
                                        g = doc.Objects.AddHatch(hatch)
                                        if g is not None and str(g) != "00000000-0000-0000-0000-000000000000":
    ...
    # MULTIPOLYGON — HATCH면 각 폴리곤을 해치로
            ...
            for poly_part in poly_parts:
                ...
                for ring_str in rings:
                    ...
                    if pts:
                        ...
                        pc = Rhino.Geometry.PolylineCurve(pl)
                        curves.append(pc)
                if curves and entity_type == "HATCH":
                    try:
                        pattern_index = doc.HatchPatterns.Find("Solid", True)
                        if pattern_index < 0:
                            pattern_index = doc.HatchPatterns.Find("Grid", True)
...
def _do_open_from_info_impl(info):
    ...
    total = _entities_to_rhino(entities, layer_colors)
    print("도면 불러오기 완료. 객체 {}개 추가됨.".format(total))
    _debug("추가된 객체 수: {}".format(total))
...
def _rhino_geometry_to_wkt(geom):
    """Rhino Geometry -> (geom_wkt, entity_type, props_extra). 변환 불가 시 (None, None, None)."""
    if geom is None:
        return (None, None, None)

    def _append_valid_point(dst, pt):
        import math
        try:
            x = float(pt.X)
            y = float(pt.Y)
            ...
            if math.isinf(x) or math.isinf(y) or math.isinf(z):
                return
            if dst and abs(dst[-1][0] - x) <= 1e-9 and abs(dst[-1][1] - y) <= 1e-9 and abs(dst[-1][2] - z) <= 1e-9:
                return
            dst.append((x, y, z))
        except Exception:
            return

    def _sample_curve_to_linestring(curve, segs_hint=32):
        """Curve를 샘플링해 LINESTRING Z로 변환. 실패 시 None."""
        try:
            segs = int(segs_hint or 32)
            if segs < 8:
                segs = 8
            if segs > 512:
                segs = 512
            ...
            if domain is not None and hasattr(domain, "ParameterAt"):
                for i in range(segs + 1):
                    t = domain.ParameterAt(float(i) / float(segs))
                    p = curve.PointAt(t)
                    _append_valid_point(pts, p)
            else:
                tvals = curve.DivideByCount(segs, True)
                if tvals:
                    for t in tvals:
                        p = curve.PointAt(t)
                        _append_valid_point(pts, p)
            if len(pts) < 2:
                try:
                    pl = curve.TryGetPolyline()
                    if pl and len(pl) >= 2 and pl[0] and pl[1] is not None:
                        for p in pl[1]:
                            _append_valid_point(pts, p)
                except Exception:
                    pass
            if len(pts) < 2:
                try:
                    ...
                    if length > 0:
                        seg_len = max(length / float(segs), 1e-4)
                        tvals = curve.DivideByLength(seg_len, True)
                        if tvals:
                            for t in tvals:
                                p = curve.PointAt(t)
                                _append_valid_point(pts, p)
            ...
            if len(pts) < 2:
                try:
                    ...
                    if nc is not None:
                        tvals = nc.DivideByCount(segs, True)
                        if tvals:
                            for t in tvals:
                                p = nc.PointAt(t)
                                _append_valid_point(pts, p)
            ...
            if len(pts) < 2:
                ...
                p1 = curve.PointAtEnd
                _append_valid_point(pts, p0)
                _append_valid_point(pts, p1)
            if len(pts) < 2:
                return None
            try:
                if getattr(curve, "IsClosed", False) and len(pts) >= 3:
                    if abs(pts[0][0] - pts[-1][0]) > 1e-9 or abs(pts[0][1] - pts[-1][1]) > 1e-9 or abs(pts[0][2] - pts[-1][2]) > 1e-9:
                        pts.append(pts[0])
            except Exception:
                pass
            return _linestring_wkt(pts)
        except Exception:
            return None
    ...
    try:
        ...
        if isinstance(geom, Rhino.Geometry.ArcCurve):
            try:
                ...
                if arc.IsCircle:
                    ...
                    pts = []
                    for i in range(segs + 1):
                        t = float(i) / segs * 2.0 * __import__("math").pi
                        p = arc.PointAt(t)
                        pts.append((p.X, p.Y, p.Z))
                    return (_linestring_wkt(pts), "CIRCLE", None)
            except Exception:
                pass
            # 열린 ArcCurve는 curve domain 기준으로 샘플링한다.
            try:
                length = float(geom.GetLength())
                segs = min(256, max(16, int(length / 0.1) if length > 0 else 32))
            except Exception:
                segs = 64
            wkt_arc = _sample_curve_to_linestring(geom, segs)
            if wkt_arc:
                return (wkt_arc, "ARC", None)
        # Arc-like Curve (e.g. GH open curve recognized as arc)
        if isinstance(geom, Rhino.Geometry.Curve):
            try:
                arc_try = geom.TryGetArc()
                if arc_try and len(arc_try) >= 2 and arc_try[0] and arc_try[1] is not None:
                    ac = Rhino.Geometry.ArcCurve(arc_try[1])
                    try:
                        length = float(ac.GetLength())
                        segs = min(256, max(16, int(length / 0.1) if length > 0 else 32))
                    except Exception:
                        segs = 64
                    wkt_arc = _sample_curve_to_linestring(ac, segs)
                    if wkt_arc:
                        return (wkt_arc, "ARC", None)
            except Exception:
                pass
        # Circle (not ArcCurve)
        if type(geom).__name__ == "CircleCurve" or (hasattr(geom, "Circle") and geom.Circle is not None):
            try:
                circle = geom.Circle if hasattr(geom, "Circle") else geom
                cen = circle.Center
                r = circle.Radius
                import math
                pts = []
                for i in range(65):
                    ...
                    pts.append((cen.X + r * math.cos(t), cen.Y + r * math.sin(t), cen.Z))
                return (_linestring_wkt(pts), "CIRCLE", None)
            except Exception:
                pass
        # Point
        ...
        # Hatch: 경계를 LINESTRING Z(닫힌) 또는 POLYGON으로
                ...
                if curves and len(curves) > 0:
                    ...
                    for c in curves:
                        if c.TryGetPolyline()[0]:
                            ...
                            segs = 32
                            for i in range(segs + 1):
                                t = c.Domain.ParameterAt(float(i) / segs)
                                p = c.PointAt(t)
                                all_pts.append((p.X, p.Y, p.Z))
                    if all_pts:
                        wkt = _linestring_wkt(all_pts)
                        return (wkt, "HATCH", None)
            except Exception:
                pass
        # 일반 Curve: 샘플링
        if isinstance(geom, Rhino.Geometry.Curve):
            try:
                try:
                    ...
                    segs = 64
                wkt = _sample_curve_to_linestring(geom, segs)
                if wkt:
                    return (wkt, "LWPOLYLINE", None)
            except Exception:
                pass
...
def _get_instance_xform_params(obj):
    ...
    try:
        ...
        if xform is not None:
            # 4x4 행렬: 열벡터 기준 X=(M00,M10,M20), Y=(M01,M11,M21), Z=(M02,M12,M22), 이동=(M03,M13,M23)
...
def _entity_list_bbox_min(entities):
    """엔티티 리스트에서 전체 bbox의 (min_x, min_y, min_z) 반환. 라이노 위치 그대로 업로드."""
...
def _is_instance_object(obj):
    ...
    if obj is None:
        return False
    try:
        # 1) isinstance가 가장 확실함 (Rhino.DocObjects.InstanceObject)
        if hasattr(Rhino.DocObjects, "InstanceObject"):
            if isinstance(obj, Rhino.DocObjects.InstanceObject):
                return True
        # 2) InstanceDefinition 속성 존재 여부
        if getattr(obj, "InstanceDefinition", None) is not None:
            return True
        # 3) 타입명 체크 (네임스페이스 포함 대응)
...
def _rhino_doc_to_entity_list(doc):
    """문서를 엔티티 + 블록 정의/배치로 변환. 반환: {"entities": [...], "block_defs": [...], "block_inserts": [...]}."""
    if doc is None:
        return {"entities": [], "block_defs": [], "block_inserts": []}
    out = []
    instance_objs = []
    layer_colors = {}
    try:
        for obj in doc.Objects:
            if obj.IsDeleted or obj.Geometry is None:
                continue
            # 블록 참조: InstanceObject 타입 또는 InstanceReferenceGeometry 지오메트리
            ...
            if is_block:
                ...
                try:
                    ...
                    blk_name = getattr(idef, "Name", "?") if idef else "?"
                    _debug("블록 인식: {} (누적 {})".format(blk_name, len(instance_objs)))
                except Exception:
                    pass
                continue
            try:
            ...
            try:
                ...
                if bbox.IsValid:
                    ...
                    pts = [(bbox.Min.X, bbox.Min.Y, bbox.Min.Z), (bbox.Max.X, bbox.Max.Y, bbox.Max.Z)]
                    xs = [p[0] for p in pts]
                    ys = [p[1] for p in pts]
...
def CadManageSave(commit_options=None):
    ...
    먼저 CadManageOpen 으로 불러온 문서여야 합니다.
    commit_options: dict (선택) - version_label, assignee_name, assignee_department, change_notes,
        class_pre, class_major, class_mid, class_minor, class_work_type, settings(JSON문자열) 등.
    """
    if scriptcontext.doc is None:
    ...
    link = _load_link()
    if not link:
        print("연동 정보가 없습니다. 먼저 CadManageOpen 으로 웹에서 버전을 불러오세요.")
        return

    api_base = (link.get("api_base") or "").rstrip("/")
    project_id = link.get("project_id")
    parent_commit_id = link.get("parent_commit_id")
    if not api_base or project_id is None:
        print("연동 정보가 올바르지 않습니다. CadManageOpen 을 다시 실행하세요.")
        return
    ...
    try:
        ...
        payload = {
            ...
            "settings": opts.get("settings") if isinstance(opts.get("settings"), dict) else (json.loads(opts["settings"]) if opts.get("settings") and isinstance(opts.get("settings"), str) else None),
        }
...
def _activate_rhino_view_and_zoom_extents():
    """Rhino 메인 창 활성화 후 활성 뷰에서 ZoomExtents 및 Redraw."""
    try:
...
if eto_forms is not None and eto_drawing is not None:
    class CadManageEtoDialog(eto_forms.Form):
        ...
        def __init__(self):
            ...
            self.Resizable = True
            self.ClientSize = eto_drawing.Size(400, 420)
            self._projects = []
            self._commits = []
            self._users = []
            self._departments = []
            ...
            self._lbl_commit_settings.Text = "커밋 설정 (저장 시 신규 버전으로 DB 업로드)"
            ...
            self._btn_import.MinimumSize = eto_drawing.Size(100, 26)
            ...
            load_layout.AddRow(self._lbl_commit, self._drop_commit)
            load_layout.AddRow(None)
            load_layout.AddRow(self._btn_import)
